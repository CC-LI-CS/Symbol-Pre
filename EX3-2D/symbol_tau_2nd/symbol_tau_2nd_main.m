...
stxfcol=[Txseries(2);0.5*(Txseries(1)+Txseries(3));0.5*Txseries(4:end)];%(G+G^T)/2
tauxeigs=get_taumateig(stxfcol);
taux=(d1p*d1m)*tauxeigs.^2;
...
styfcol=[Tyseries(2);0.5*(Tyseries(1)+Tyseries(3));0.5*Tyseries(4:end)];
tauyeigs=get_taumateig(styfcol);
tauy=(d2p*d2m)*tauyeigs.^2;
...
lam_L1=(d1p-d1m)^2*(lam_L.^alpha1+(alpha1^2-2*alpha1)/4*lam_L.^(alpha1+1));

lam_L2=(d2p-d2m)^2*(lam_L.^alpha2+(alpha2^2-2*alpha2)/4*lam_L.^(alpha2+1));

lx=sqrt(lam_L1+taux);

ly=(h1^alpha1)/(h2^alpha2)*sqrt(lam_L2+tauy);

%% preconditoner eigenvalue
s1fac=sqrt(2/(N1+1));s2fac=sqrt(2/(N2+1));
scasqrtaueigs=nu+(kron(ones(N2,1),lx)+kron(ly,ones(N1,1)));
